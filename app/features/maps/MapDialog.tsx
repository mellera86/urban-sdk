...
const MapDataVisualization = dynamic(() => import("./MapDataVisualization"), {
  ...
  loading: () => (
    <div
      className="flex h-[min(60vh,600px)] items-center justify-center"
      role="status"
      aria-live="polite"
      aria-busy="true"
    >
  ...
  const handleOpenChange = (nextOpen: boolean) => {
    ...
    if (!nextOpen) {
      ...
      queryClient.removeQueries({
        queryKey: [QUERY_KEYS.MAPS_CONFIG, configUrl],
      });
    }
  };
        ...
        <DialogTrigger
          ...
          render={
            <button
              type="button"
              className={cn(
                "block h-full w-full min-w-0 cursor-pointer rounded-2xl border-0 bg-transparent p-0 text-left",
        ...
        <DialogContent className="flex w-[min(96vw,960px)] max-w-none flex-col gap-4 sm:max-w-none">
          ...
          {isPending && (
            <div
              ...
              aria-busy="true"
            >
              <p className="sr-only">Loading map data</p>
              <Spinner className="size-10" />
            </div>
          )}
